...
const isOvernightStrategy = (strategy) => strategy === "gold_overnight_long" || isEtfOvernightLongStrategy(strategy);
...
const GOLD_SESSION_NAMES = {
  ...
  gold_intraday_close_momentum: "Gold Intraday Close Momentum v1",
  gold_abnormal_momentum: "Gold Abnormal Momentum v1",
  asia_session_long: "Asia Session Long v1",
  shanghai_day_long: "Shanghai Day Long v1",
  gold_overnight_long: "Gold Overnight Long v1",
...
const GOLD_SESSION_LEGS = {
  ...
  gold_intraday_close_momentum: "gld_fifth_half_hour_momentum",
  gold_abnormal_momentum: "abnormal_momentum",
  asia_session_long: "asia_long",
  shanghai_day_long: "shanghai_day_long",
  gold_overnight_long: "overnight_long",
  comex_day_short: "day_short",
};
const goldSessionStrategyName = (strategy) => GOLD_SESSION_NAMES[strategy] || "COMEX Day Short v1";
const goldSessionLeg = (strategy) => GOLD_SESSION_LEGS[strategy] || "day_short";
...
function updateBacktestAvailability() {
  ...
  if (chronological) {
    ...
    if (!ready) {
      $("#resolutionNote").textContent = trend
        ? `This test is locked until M1, ${trendTimeframe(strategy)} and D1 Market Memory all reach 100%.`
        : "This test is locked until M1 Market Memory reaches 100%.";
    } else if (trend) {
      ...
      const sessionNotes = {
        ...
        gold_intraday_close_momentum: "M1 Market Memory is ready. EVE follows the 11:30-12:00 New York move once at 15:30 and exits at 16:00.",
        gold_abnormal_momentum: "M1 Market Memory is ready. EVE checks one causal two-sigma gold signal per GMT+3 day and never takes more than one trade.",
        asia_session_long: "M1 Market Memory is ready. EVE buys once at 18:00 New York and closes at the associated 15:30 Shanghai gold-session close.",
        shanghai_day_long: "M1 Market Memory is ready. EVE buys once at the official 09:00 Shanghai day open and closes at 15:30 Shanghai.",
        gold_overnight_long: "M1 Market Memory is ready. EVE buys once at 13:30 New York, includes financing, and closes at the next eligible 08:20 open.",
        comex_day_short: "M1 Market Memory is ready. EVE sells once at 08:20 New York and closes at 13:30 the same day.",
      };
      $("#resolutionNote").textContent = sessionNotes[strategy];
      ...
      $("#resolutionNote").textContent = "M1 Market Memory is ready. EVE reconstructs completed M5 candles, applies London daylight-saving time, sizes risk, and includes spread and commission.";
    } else if (isNewYorkStrategy(strategy)) {
      $("#resolutionNote").textContent = "M1 Market Memory is ready. EVE requires every 08:30–09:00 New York minute, permits one 09:00 entry only, and includes risk sizing and costs.";
    } else {
      $("#resolutionNote").textContent = "M1 Market Memory is ready. The signal candle must close before EVE enters at the next candle open. Costs and the zero-balance account stop are included.";
    }
  } else {
...
function updateTesterForm() {
  ...
  const trend = isTrendStrategy(strategy);
  const london = isLondonStrategy(strategy);
  const newYork = isNewYorkStrategy(strategy);
  const comex = isComexStrategy(strategy);
  ...
  const researchBalance = trend || london || newYork || comex || goldSession;
  const chronological = isChronologicalStrategy(strategy);
  ...
  document.querySelectorAll("[data-comex-field]").forEach((node) => { node.hidden = !comex; });
  document.querySelectorAll("[data-gold-session-field]").forEach((node) => { node.hidden = !goldSession; });
  document.querySelectorAll("[data-overnight-field]").forEach((node) => { node.hidden = !(goldSession && overnight); });
  document.querySelectorAll("[data-session-risk-field]").forEach((node) => { node.hidden = !(london || newYork); });
  ...
  document.querySelectorAll("[data-non-london-field]").forEach((node) => { node.hidden = researchBalance; });
  ...
  if (goldSession) {
    ...
    const sessionDetails = {
      ...
      gold_intraday_close_momentum: "11:30-12:00 New York direction → one 15:30 trade → close 16:00",
      gold_abnormal_momentum: "Previous 60 GMT+3 daily returns → two-sigma move → one late-day momentum trade → close 23:59",
      asia_session_long: "Buy 18:00 New York → eastern-session hold → close 15:30 Shanghai → no rollover charge",
      shanghai_day_long: "Buy 09:00 Shanghai → official SGE day session → close 15:30 Shanghai → no rollover charge",
      gold_overnight_long: "Buy 13:30 New York → hold overnight → close next eligible 08:20 → financing and hard stop included",
    ...
    const sessionRuleStrips = {
      ...
      gold_intraday_close_momentum: "<span>11:30-12:00 DIRECTION</span><span>15:30 ENTRY</span><span>ONE TRADE EVERY COMPLETE WEEKDAY</span><span>FIXED 0.01 LOT</span><span>0.25% HARD STOP</span><span>16:00 EXIT</span>",
      gold_abnormal_momentum: "<span>60 PRIOR DAYS</span><span>2-SIGMA TRIGGER</span><span>17:00 SHORT / 19:00 BUY</span><span>MAX 1 TRADE/DAY</span><span>0.25% HARD STOP</span><span>23:59 GMT+3 EXIT</span>",
      asia_session_long: "<span>BUY 18:00 NEW YORK</span><span>EASTERN SESSION</span><span>MAX 1 TRADE/DAY</span><span>FIXED 0.01 LOT</span><span>0.25% HARD STOP</span><span>15:30 SHANGHAI EXIT</span>",
      shanghai_day_long: "<span>BUY 09:00 SHANGHAI</span><span>SGE DAY SESSION</span><span>MAX 1 TRADE/DAY</span><span>FIXED 0.01 LOT</span><span>0.25% HARD STOP</span><span>15:30 SHANGHAI EXIT</span>",
      gold_overnight_long: "<span>BUY 13:30 NEW YORK</span><span>OVERNIGHT HOLD</span><span>MAX 1 TRADE/DAY</span><span>FIXED 0.01 LOT</span><span>0.25% HARD STOP</span><span>NEXT 08:20 EXIT</span>",
      comex_day_short: "<span>SELL 08:20 NEW YORK</span><span>DAY SESSION</span><span>MAX 1 TRADE/DAY</span><span>FIXED 0.01 LOT</span><span>0.25% HARD STOP</span><span>13:30 EXIT</span>",
    };
    setText("#testerSourceDetail", sessionDetails[strategy]);
    setHtml("#testerRuleStrip", sessionRuleStrips[strategy]);
  } else if (trend) {
    ...
    setText("#testerSourceDetail", `60-day direction → 55-${trendFrame} breakout → first M1 open → 2 ATR stop → 20-${trendFrame} trailing exit`);
    setHtml("#testerRuleStrip", `<span>60-DAY DIRECTION</span><span>55-${trendFrame} BREAKOUT</span><span>FIRST M1 OPEN</span><span>0.25% RISK</span><span>2 ATR STOP</span><span>20-${trendFrame} EXIT</span>`);
  } else if (comex) {
    setText("#testerFormTitle", "COMEX Closing Momentum v1");
    setText("#testerSourceName", "EVE COMEX Closing Momentum v1 · M1 replay");
    setText("#testerSourceDetail", "Prior 13:29 settlement proxy → follow direction at 13:00 → 0.25% hard stop → 13:30 exit");
    setHtml("#testerRuleStrip", "<span>PRIOR 13:29 REFERENCE</span><span>13:00 ENTRY</span><span>MAX 1 TRADE/DAY</span><span>FIXED 0.01 LOT</span><span>0.25% HARD STOP</span><span>13:30 EXIT</span>");
  } else if (newYork) {
    setText("#testerFormTitle", "New York Morning Momentum v1");
    setText("#testerSourceName", "EVE New York Morning Momentum v1 · M1 replay");
    setText("#testerSourceDetail", "08:30–09:00 New York direction → 09:00 entry → opposite-edge stop → 15:55 exit");
    setHtml("#testerRuleStrip", "<span>08:30–09:00 NEW YORK</span><span>09:00 ENTRY</span><span>MAX 1 TRADE/DAY</span><span>0.25% RISK</span><span>RANGE-EDGE STOP</span><span>15:55 EXIT</span>");
  } else if (london) {
    setText("#testerFormTitle", "London Opening Range v1");
    setText("#testerSourceName", "EVE London Opening Range v1 · M5 signal / M1 replay");
    setText("#testerSourceDetail", "08:00–08:30 London range → confirmed M5 breakout → next M5 open → midpoint stop → 2R");
    setHtml("#testerRuleStrip", "<span>08:00–08:30 LONDON</span><span>M5 CLOSE CONFIRMS</span><span>NEXT M5 OPEN</span><span>0.25% RISK</span><span>MIDPOINT STOP</span><span>2R TARGET</span>");
  } else if (liquidity) {
    setText("#testerFormTitle", liquidityStrategyName(strategy));
    setText("#testerSourceName", `EVE ${liquidityStrategyName(strategy)} · Python M1 replay`);
    setText("#testerSourceDetail", continuation
    ...
    setHtml("#testerRuleStrip", continuation
    ...
    setText("#testerFormTitle", "Fixed Ladder v2.61");
    setText("#testerSourceName", "EVE_Twelve_Data_Fixed_Ladder_v2.61.mq5");
    setText("#testerSourceDetail", "Source-verified legacy reconstruction · SHA-256 f033bc756b8a…d02da9");
    setHtml("#testerRuleStrip", "<span>8 BUY STOP</span><span>8 SELL STOP</span><span>3.000 SPACING</span><span>2.000 FALLBACK</span><span>0.750 FIRST CUT</span><span>BE +0.150 AT +1.500</span>");
  }
  ...
  if (researchBalance && Number($("#startingBalance")?.value || 0) === 1000) $("#startingBalance").value = "10000";
  if (!researchBalance && Number($("#startingBalance")?.value || 0) === 10000) $("#startingBalance").value = "1000";
...
function backtestPayload() {
  ...
  if (isGoldSessionStrategy(strategy)) {
    const testSegment = $("#testPeriod")?.value || "development";
    const startDate = $("#testDateFrom")?.value || "";
    const endDate = $("#testDateTo")?.value || "";
    return {
      ...
      session_leg: goldSessionLeg(strategy),
      symbol: "XAU/USD",
      interval: "1min",
      resolution: "m1_replay",
      test_segment: testSegment,
      ...
      asia_entry_hour: 18,
      asia_entry_minute: 0,
      asia_exit_timezone_name: "Asia/Shanghai",
      shanghai_entry_hour: 9,
      shanghai_entry_minute: 0,
      asia_exit_hour: 15,
      asia_exit_minute: 30,
      abnormal_timezone_name: "Etc/GMT-3",
      abnormal_lookback_days: 60,
      abnormal_sigma: 2.0,
      abnormal_negative_entry_hour: 17,
      abnormal_negative_entry_minute: 0,
      abnormal_positive_entry_hour: 19,
      abnormal_positive_entry_minute: 0,
      abnormal_exit_hour: 23,
      abnormal_exit_minute: 59,
      intraday_predictor_start_hour: 11,
      ...
      intraday_exit_minute: 0,
      etf_market_open_hour: 9,
      etf_market_open_minute: 30,
      etf_market_close_hour: 16,
      etf_market_close_minute: 0,
      long_overnight_cost_per_001_lot: Number($("#goldSessionOvernightCost").value),
      triple_swap_weekday: 2,
      spread_price: Number($("#spreadPrice").value),
      commission_per_001_lot: Number($("#commission").value),
      slippage_price: Number($("#slippagePrice").value),
  ...
  if (isComexStrategy(strategy)) {
    const testSegment = $("#testPeriod")?.value || "development";
    const startDate = $("#testDateFrom")?.value || "";
    const endDate = $("#testDateTo")?.value || "";
    return {
      strategy,
      name: "COMEX Closing Momentum v1",
      symbol: "XAU/USD",
      interval: "1min",
      resolution: "m1_replay",
      test_segment: testSegment,
  ...
  if (isNewYorkStrategy(strategy)) {
    const testSegment = $("#testPeriod")?.value || "development";
    const startDate = $("#testDateFrom")?.value || "";
    const endDate = $("#testDateTo")?.value || "";
    return {
      strategy,
      name: "New York Morning Momentum v1",
      symbol: "XAU/USD",
      interval: "1min",
      resolution: "m1_replay",
      test_segment: testSegment,
...
async function runBacktest(button) {
  ...
  try {
    const endpoint = isTrendStrategy(payloadBody.strategy)
      ...
      : (isGoldSessionStrategy(payloadBody.strategy)
      ? "backtests/gold-session-anomaly"
      : (isComexStrategy(payloadBody.strategy)
      ? "backtests/comex-closing-momentum"
      : (isNewYorkStrategy(payloadBody.strategy)
...
function backtestResolutionLabel(run = {}) {
  ...
  if (isGoldSessionStrategy(strategy)) {
    ...
    return `${goldSessionStrategyName(strategy)} · ${segments[run.reliability?.test_segment || run.settings?.test_segment] || "M1 replay"}`;
  }
  if (isComexStrategy(strategy)) {
    const segments = { full: "Full M1 history", development: "Development first ⅔", untouched: "Untouched final ⅓", custom: "Custom M1 period" };
    return `COMEX Closing Momentum v1 · ${segments[run.reliability?.test_segment || run.settings?.test_segment] || "M1 replay"}`;
  }
  if (isNewYorkStrategy(strategy)) {
    const segments = { full: "Full M1 history", development: "Development first ⅔", untouched: "Untouched final ⅓", custom: "Custom M1 period" };
    return `New York Morning Momentum v1 · ${segments[run.reliability?.test_segment || run.settings?.test_segment] || "M1 replay"}`;
  }
  if (isLondonStrategy(strategy)) {
    const segments = { full: "Full M1 history", development: "Development first ⅔", untouched: "Untouched final ⅓", custom: "Custom M1 period" };
...
function renderComparison(runs = []) {
  ...
  if (m5 && m1) {
    ...
    if (isTrendStrategy(strategy)) {
      ...
      settingsKeys = [
        "strategy", "session_leg", "symbol", "starting_balance", "fixed_lot", "maximum_loss_percent", "timezone_name",
        "day_open_hour", "day_open_minute", "settlement_hour", "settlement_minute", "asia_entry_hour", "asia_entry_minute",
        "asia_exit_timezone_name", "shanghai_entry_hour", "shanghai_entry_minute", "asia_exit_hour", "asia_exit_minute", "long_overnight_cost_per_001_lot",
        "abnormal_timezone_name", "abnormal_lookback_days", "abnormal_sigma", "abnormal_negative_entry_hour",
      ...
      settingsKeys = [
        "strategy", "symbol", "starting_balance", "fixed_lot", "maximum_loss_percent", "timezone_name",
        "reference_hour", "reference_minute", "entry_hour", "entry_minute", "exit_hour", "exit_minute",
        "spread_price", "commission_per_001_lot", "slippage_price", "money_per_price_per_001_lot", "path_mode",
      ];
    } else if (isNewYorkStrategy(strategy)) {
      settingsKeys = [
        "strategy", "symbol", "starting_balance", "risk_percent", "minimum_lot", "lot_step", "maximum_lot",
        "timezone_name", "signal_start_hour", "signal_start_minute", "signal_minutes", "entry_hour", "entry_minute",
        "force_exit_hour", "force_exit_minute", "spread_price", "commission_per_001_lot", "slippage_price",
        "money_per_price_per_001_lot", "path_mode",
      ];
    } else if (isLondonStrategy(strategy)) {
      settingsKeys = [
