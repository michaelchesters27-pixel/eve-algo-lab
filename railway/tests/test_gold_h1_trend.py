...
from app.models.schemas import GoldH1TrendBacktestRequest
from app.services.backtests import BacktestService, gold_h1_settings_match
...
def signal_event(
    ...
    *,
    ...
    return TrendBarEvent(
        event_time=at,
        h1_bar_time=at - timedelta(hours=1),
        h1_close=close,
...
def trend_history() -> tuple[list[Candle], list[Candle], datetime]:
    h1_start = datetime(2026, 3, 1, tzinfo=UTC)
    h1 = [candle(h1_start + timedelta(hours=index), 100.0, 101.0, 99.0, 100.0) for index in range(55)]
    h1.append(candle(h1_start + timedelta(hours=55), 100.0, 103.0, 99.5, 102.0))
    event_time = h1[-1].candle_time + timedelta(hours=1)
    ...
    return h1, daily, event_time


def zero_cost_parameters(**overrides) -> GoldH1TrendParameters:
    values = {
        "spread_price": 0.0,
    ...
    return GoldH1TrendParameters(**values)


def test_completed_h1_breakout_requires_matching_60_day_direction() -> None:
    h1, daily, event_time = trend_history()

    events = build_trend_events(h1, daily, zero_cost_parameters())
    ...
    rejected = build_trend_events(h1, falling_daily, zero_cost_parameters())
...
def test_risk_sizing_rounds_down_and_hard_stop_is_replayed_on_m1() -> None:
    ...
    simulator = GoldH1TrendBacktester(10_000.0, zero_cost_parameters(), [signal_event(at)])
...
def test_completed_h1_channel_exit_closes_at_next_m1_open() -> None:
    at = datetime(2026, 3, 10, 12, tzinfo=UTC)
    exit_event = signal_event(at + timedelta(hours=1), side=None, close=94.0, exit_low=95.0)
    simulator = GoldH1TrendBacktester(10_000.0, zero_cost_parameters(), [signal_event(at), exit_event])

    simulator.process_candle(candle(at, 100.0, 101.0, 99.0, 100.0))
    simulator.process_candle(candle(at + timedelta(hours=1), 110.0, 111.0, 109.0, 110.0))

    summary = simulator.summary()
    assert summary.total_positions == 1
    assert summary.ending_balance == pytest.approx(10_060.0)
    assert summary.channel_exits == 1
    assert summary.exit_reasons == {"20-H1 CHANNEL EXIT": 1}
...
def test_wednesday_overnight_financing_is_charged_three_times() -> None:
    ...
    simulator = GoldH1TrendBacktester(10_000.0, params, [signal_event(wednesday)])
...
def test_request_defaults_and_untouched_lock_freeze_every_trend_input() -> None:
    request = GoldH1TrendBacktestRequest()
    assert request.starting_balance == 10_000.0
    assert request.entry_lookback_h1 == 55
    assert request.exit_lookback_h1 == 20
    assert request.daily_trend_lookback == 60
    assert request.atr_period_h1 == 20
    ...
    baseline["strategy"] = "gold_h1_trend"
    untouched = {**baseline, "test_segment": "untouched"}
    assert gold_h1_settings_match(baseline, untouched)
    assert not gold_h1_settings_match(baseline, {**untouched, "risk_percent": 0.50})
    assert not gold_h1_settings_match(baseline, {**untouched, "overnight_long_cost_per_001_lot": 0.0})
    assert not gold_h1_settings_match(baseline, {**untouched, "entry_lookback_h1": 45})
...
@pytest.mark.asyncio
async def test_service_loads_h1_and_d1_then_persists_m1_replayed_trade() -> None:
    h1, daily, event_time = trend_history()
    m1 = [candle(event_time, 100.0, 101.0, 95.0, 96.0)]
    repo = FakeTrendRepository({"1h": rows(h1), "1day": rows(daily), "1min": rows(m1)})
    service = BacktestService(repo)  # type: ignore[arg-type]
    request = {
        **GoldH1TrendBacktestRequest(test_segment="development").model_dump(mode="json"),
