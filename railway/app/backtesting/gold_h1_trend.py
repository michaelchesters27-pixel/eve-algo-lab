...
class GoldH1TrendParameters:
    entry_lookback_h1: int = 55
    exit_lookback_h1: int = 20
    daily_trend_lookback: int = 60
    atr_period_h1: int = 20
    ...
    def validate(self) -> None:
        if self.entry_lookback_h1 < 2:
            raise ValueError("H1 entry lookback must be at least two bars")
        if self.exit_lookback_h1 < 2:
            raise ValueError("H1 exit lookback must be at least two bars")
        if self.daily_trend_lookback < 2:
            raise ValueError("Daily trend lookback must be at least two bars")
        if self.atr_period_h1 < 2:
            raise ValueError("H1 ATR period must be at least two bars")
...
class GoldTrendCompletedTrade:
    ...
    def _metadata(self) -> dict[str, Any]:
        return {
            ...
            "h1_bar_time": self.signal.h1_bar_time.isoformat(),
            ...
            "atr_h1": round(self.signal.atr_h1, 10),
            "planned_risk_money": round(self.planned_risk_money, 6),
            "overnight_financing": round(self.financing_costs, 6),
            "entry_protocol": "completed H1 55-bar breakout with matching 60-trading-day direction; entry at first available M1 open",
            "exit_protocol": "hard 2x H1 ATR stop or completed H1 close through the opposite 20-bar channel",
        }
...
def build_trend_events(
    ...
    parameters: GoldH1TrendParameters,
    *,
    ...
    Twelve Data timestamps identify the start of H1 and D1 candles. An H1
    decision therefore becomes tradable one hour later, and a daily close
    becomes available one day after its stored start timestamp.
    """

    parameters.validate()
    h1 = _normalise_candles(h1_candles)
    ...
    warmup = max(parameters.entry_lookback_h1, parameters.exit_lookback_h1, parameters.atr_period_h1)
    events: list[TrendBarEvent] = []

    for index in range(warmup, len(h1)):
        current = h1[index]
        event_time = current.candle_time + timedelta(hours=1)
        if date_from is not None and event_time < date_from:
            continue
        if date_to is not None and event_time > date_to:
            break

        entry_window = h1[index - parameters.entry_lookback_h1 : index]
        exit_window = h1[index - parameters.exit_lookback_h1 : index]
        ...
        atr_start = index - parameters.atr_period_h1 + 1
        true_ranges = [_true_range(h1[position], h1[position - 1]) for position in range(atr_start, index + 1)]
        atr_h1 = sum(true_ranges) / len(true_ranges)
        entry_side = raw_side if raw_side is not None and raw_side == daily_side else None
        events.append(
            TrendBarEvent(
                event_time=event_time,
                h1_bar_time=current.candle_time,
                h1_close=current.close,
...
class GoldH1TrendBacktester:
    """H1/D1 trend decisions with M1 execution, gap and stop replay."""
    ...
    def _open_from_event(self, event: TrendBarEvent, at: datetime, mid: float) -> bool:
        ...
        if side is None or self.position is not None or self.account_ruined:
            return False
        if event.daily_close is None or event.daily_reference_close is None or event.atr_h1 <= self.EPS:
            self.signals_filtered += 1
            return False

        entry_price = self._entry_price(side, mid)
        stop_distance = event.atr_h1 * self.params.atr_multiplier
        ...
        signal = GoldTrendSignal(
            ...
            h1_bar_time=event.h1_bar_time,
            signal_close=event.h1_close,
            atr_h1=event.atr_h1,
        ...
        self.position = GoldTrendPosition(
            position_id=f"GOLD-H1-POS-{self._sequence:08d}",
            basket_id=f"GOLD-H1-TRADE-{self._sequence:08d}",
            side=side,
            opened_at=at,
    ...
    def _process_due_events(self, at: datetime, mid: float) -> None:
        while self._event_index < len(self.events) and self.events[self._event_index].event_time <= at:
            ...
            if self.position is not None:
                channel_exit = (
                    self.position.side == "buy" and event.h1_close < event.exit_channel_low
                ) or (
                    self.position.side == "sell" and event.h1_close > event.exit_channel_high
                )
                if channel_exit:
                    self.channel_exits += 1
                    self._close_position(at, mid, "20-H1 CHANNEL EXIT")
    ...
    def summary(self) -> GoldTrendSimulationSummary:
        ...
        return GoldTrendSimulationSummary(
            ...
            h1_events_processed=self.h1_events_processed,
